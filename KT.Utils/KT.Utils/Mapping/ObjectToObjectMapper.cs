...
namespace KT.Utils.Mapping
{
  ...
  public class ObjectToObjectMapper
  {
    ...
    public static ClassMapping CreateAutoMapping<T, U>()
    {
      return CreateAutoMapping<T, U>(new AutomappingSettings() { IncludeReferenceTypes = false });
    }

    public static ClassMapping CreateAutoMapping<T, U>(AutomappingSettings settings)
    {
      bool includeReferenceTypes = settings.IncludeReferenceTypes;
      ...
      gpa.AddPropertiesFromType(t1);
      gpa.AddPropertiesFromType(t2);

      ClassMapping mapping = new ClassMapping(t1, t2);

      IEnumerable<PropertyInfo> pInfosU = gpa.GetPropertyInfosByType(t2);

      foreach (PropertyInfo piT in gpa.GetPropertyInfosByType(t1).Where(x => (settings.ExcludeProperties == null || !settings.ExcludeProperties.Contains(x.Name)) && ((settings.IncludeProperties != null && settings.IncludeProperties.Contains(x.Name)) || includeReferenceTypes || x.PropertyType.IsValueType || x.PropertyType.Name == TypeNameString)))
      {
        PropertyInfo piU = pInfosU.Where(x => includeReferenceTypes || x.PropertyType.IsValueType || x.PropertyType.Name == TypeNameString).SingleOrDefault(x => x.Name == piT.Name);

        if (piU != null)
        {
          mapping.FieldMappings.AddMapping(new FieldMapping(new FieldMappingInfo(piT.Name, piT.PropertyType), new FieldMappingInfo(piU.Name, piU.PropertyType)));
        }
      }
