Imports System.ComponentModel

Public Class ArkistonSijainti

#Region "Hakumetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Select, True)>
    Public Function Hae() As List(Of Entities.hlp_ArkistonSijainti)

        Dim tietokanta = New DAL.ArkistonSijainti()
        Return tietokanta.Hae()

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Select, False)>
    Public Function Hae(id As Integer) As Entities.hlp_ArkistonSijainti

        Dim tietokanta = New DAL.ArkistonSijainti()
        Return tietokanta.Hae(id)

    End Function

#End Region

#Region "Muokkausmetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Insert, True)>
    Public Function Lisaa(tunnisteyksikonTyyppi As Entities.hlp_ArkistonSijainti) As Entities.hlp_ArkistonSijainti

        Dim tietokanta = New DAL.ArkistonSijainti()
        Return tietokanta.Lisaa(tunnisteyksikonTyyppi)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Update, True)>
    Public Function Muokkaa(muokattava As Entities.hlp_ArkistonSijainti) As Entities.hlp_ArkistonSijainti

        Dim tietokanta = New DAL.ArkistonSijainti()
        Return tietokanta.Muokkaa(muokattava)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Delete, True)>
    Public Function Poista(ASIId As Integer) As Entities.hlp_ArkistonSijainti

        Dim tietokanta = New DAL.ArkistonSijainti()
        Return tietokanta.Poista(ASIId)

    End Function

#End Region

End Class
