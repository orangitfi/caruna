Imports System.ComponentModel

Public Class PassivoinninSyy

#Region "Hakumetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Select, True)>
    Public Function Hae() As List(Of Entities.hlp_PassivoinninSyy)

        Dim tietokanta = New DAL.PassivoinninSyy()
        Return tietokanta.Hae()

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Select, False)>
    Public Function Hae(id As Integer) As Entities.hlp_PassivoinninSyy

        Dim tietokanta = New DAL.PassivoinninSyy()
        Return tietokanta.Hae(id)

    End Function

#End Region

#Region "Muokkausmetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Insert, True)>
    Public Function Lisaa(tunnisteyksikonTyyppi As Entities.hlp_PassivoinninSyy) As Entities.hlp_PassivoinninSyy

        Dim tietokanta = New DAL.PassivoinninSyy()
        Return tietokanta.Lisaa(tunnisteyksikonTyyppi)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Update, True)>
    Public Function Muokkaa(muokattava As Entities.hlp_PassivoinninSyy) As Entities.hlp_PassivoinninSyy

        Dim tietokanta = New DAL.PassivoinninSyy()
        Return tietokanta.Muokkaa(muokattava)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Delete, True)>
    Public Function Poista(PASId As Integer) As Entities.hlp_PassivoinninSyy

        Dim tietokanta = New DAL.PassivoinninSyy()
        Return tietokanta.Poista(PASId)

    End Function

#End Region

End Class
