Imports System.ComponentModel

Public Class AktiviteettiYhteystapa

#Region "Hakumetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Select, True)>
    Public Function Hae() As List(Of Entities.hlp_AktiviteettiYhteystapa)

        Dim tietokanta = New DAL.AktiviteettiYhteystapa()
        Return tietokanta.Hae()

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Select, False)>
    Public Function Hae(id As Integer) As Entities.hlp_AktiviteettiYhteystapa

        Dim tietokanta = New DAL.AktiviteettiYhteystapa()
        Return tietokanta.Hae(id)

    End Function

#End Region

#Region "Muokkausmetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Insert, True)>
    Public Function Lisaa(tunnisteyksikonTyyppi As Entities.hlp_AktiviteettiYhteystapa) As Entities.hlp_AktiviteettiYhteystapa

        Dim tietokanta = New DAL.AktiviteettiYhteystapa()
        Return tietokanta.Lisaa(tunnisteyksikonTyyppi)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Update, True)>
    Public Function Muokkaa(muokattava As Entities.hlp_AktiviteettiYhteystapa) As Entities.hlp_AktiviteettiYhteystapa

        Dim tietokanta = New DAL.AktiviteettiYhteystapa()
        Return tietokanta.Muokkaa(muokattava)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Delete, True)>
    Public Function Poista(YTAId As Integer) As Entities.hlp_AktiviteettiYhteystapa

        Dim tietokanta = New DAL.AktiviteettiYhteystapa()
        Return tietokanta.Poista(YTAId)

    End Function

#End Region

End Class
