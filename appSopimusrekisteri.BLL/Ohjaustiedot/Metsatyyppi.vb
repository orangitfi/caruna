Imports System.ComponentModel

Public Class Metsatyyppi

#Region "Hakumetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Select, True)>
    Public Function Hae() As List(Of Entities.hlp_Metsatyyppi)

        Dim tietokanta = New DAL.Metsatyyppi()
        Return tietokanta.Hae()

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Select, False)>
    Public Function Hae(id As Integer) As Entities.hlp_Metsatyyppi

        Dim tietokanta = New DAL.Metsatyyppi()
        Return tietokanta.Hae(id)

    End Function

#End Region

#Region "Muokkausmetodit"

    <DataObjectMethodAttribute(DataObjectMethodType.Insert, True)>
    Public Function Lisaa(tunnisteyksikonTyyppi As Entities.hlp_Metsatyyppi) As Entities.hlp_Metsatyyppi

        Dim tietokanta = New DAL.Metsatyyppi()
        Return tietokanta.Lisaa(tunnisteyksikonTyyppi)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Update, True)>
    Public Function Muokkaa(muokattava As Entities.hlp_Metsatyyppi) As Entities.hlp_Metsatyyppi

        Dim tietokanta = New DAL.Metsatyyppi()
        Return tietokanta.Muokkaa(muokattava)

    End Function

    <DataObjectMethodAttribute(DataObjectMethodType.Delete, True)>
    Public Function Poista(MTYId As Integer) As Entities.hlp_Metsatyyppi

        Dim tietokanta = New DAL.Metsatyyppi()
        Return tietokanta.Poista(MTYId)

    End Function

#End Region

End Class
